...
export const reviewUserParagraphSubmission = async ({
  ...
  const messages = [
    ...
    {
      role: 'user',
      content: `Review the paragraph I wrote and repeat it back to me, but with correct grammar, using Markdown to indicate changes. Try to keep it as close the original in language and structure as possible so I can learn from my mistakes. Explain your feedback as bullet points. I want to understand the language's specific grammar rules, so instead of saying something is "correct" or "proper", explain the rule that makes it so. Feedback should also highlight phrases that are not correct, and explain to me why they are wrong and how to fix them.`,
    },
    {
      role: 'user',
      content: paragraph,
